...
# 
# Sample convex regions in the plane obtained, from left to right, with one inequality condition, and a conjunction of 2, 3, or 4 inequality conditions, yielding **polygons**.
# :::
...
# 
# The situation generalizes in an obvious way to data in more dimensions. In that case we have more black dots in the inputs in  {numref}`nfn-fig`. Geometrically, non $n=3$ we deal with dividing planes and convex [polyhedrons](https://en.wikipedia.org/wiki/Polyhedron), and for $n>3$ with dividing [hyperplanes](https://en.wikipedia.org/wiki/Hyperplane) and convex [polytopes](https://en.wikipedia.org/wiki/Polytope).
# 
# ```{note}
...
# ```{important}
# A percepton with two-layers of neurons can classify points belonging to a convex region in $n$-dimensional space. 
# ```
...
# 
# ```{important}
# A perceptron with three or more layers with sufficiently many neurons can classify points belonging to **any** region in $n$-dimensional space with $n-1$-dimensional hyperplane boundaries.
# ```
...
# In the codes of this course we use the convention of {numref}`MCP2-fig`, namely, the bias is treated uniformly with the remaining signal. However, the bias notes are not included in the numbers $n_i$ defined above. In particular, a more detailed view of the fourth panel of {numref}`nfn-fig` is  
...
# 
# where $x^0$ is the input, and $x^i$ is the output leaving the neuron layer $i$, with $i=1, \dots, l$. All symbols $x^j$, $j=0, \dots, l$, are one-dimensional arrays. The bias nodes are included, hence the dimensions of $x^j$ are $n_j+1$, except for the ouput layer which has no bias node, hence $x^l$ has dimension $n_l$. In other words, the dimensions of the signal arrays are equal to the total number of nodes in each layer.
# 
...
# 
# Conditions 1-3 provide boundaries for the bigger traingle, and 4-6 for the smaller one contained in the bigger one.
# In the second neuron layer we need to realize two AND gates for conditions 1-3 and 4-6, espoectively, hence we take
# 
# 
